...
filename = args.output if args.output else 'MAB0340A-gabungWip.xls'

export = xlsExport(outlist,filename)

MFOUTLIST_DIR = os.path.join(os.path.expanduser("~"),'mfoutlist')
f_list_wip = open(os.path.join(MFOUTLIST_DIR,'mab0340a-gabungwip'), "r") 
list_wip = f_list_wip.readlines()
...
export.int_wocomma_col0 = {'col':[5,6,7,8],'comma_idx':-2}
...
raw_lines = []
for wip in list_wip:
    filename = 'MAB0340A-%s'%wip
    filename = filename.strip()
    print(filename)
    try:
        f_ = open(os.path.join(MFOUTLIST_DIR,'mab0340a-gabungwip'), "r") 
        f_.close()
        export.outlist = os.path.join(MFOUTLIST_DIR,filename)
        export.export_data()
        raw_lines.extend(export.raw_lines)
    except:
        continue
export.ws = export.wb.add_sheet('MT')
export.write_header()
export.raw_lines = raw_lines
export.sort_by()
export.write_body()
export.auto_width()
export.save_export()
...
export.int_wocomma_col0 = {'col':[],'comma_idx':0}
...
for wip in list_wip:
    ...
    try:
        ...
        continue
export.ws = export.wb.add_sheet('MH')
export.write_header()
export.raw_lines = raw_lines
export.sort_by()
export.write_body()
...
for wip in list_wip:
    ...
    try:
        ...
        continue
export.ws = export.wb.add_sheet('MHT')
export.write_header()
export.raw_lines = raw_lines
export.sort_by()
export.write_body()
...
for wip in list_wip:
    ...
    try:
        ...
        continue
export.ws = export.wb.add_sheet('HPP')
export.write_header()
export.raw_lines = raw_lines
export.sort_by()
export.write_body()
