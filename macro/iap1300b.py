...
# outlist name
if is_runxls:
    FILE        = os.path.join(OUTLIST_DIR,'IAP1300B-'+_param)
else:
    FILE        = os.path.join(OUTLIST_DIR,'IAP1300B')
...
if is_runxls:
    ...
    sys.argv = ['','--input=IAP1300B-'+_param,'--output=IAP1300B-'+_param+'.xls']
    # sys.argv = [sys.argv[0]]
    execfile(__file__)