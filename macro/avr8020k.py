...
# outlist name
FILE        = os.path.join(OUTLIST_DIR,'AVR8020K')

# jcl mainframe name
JCL         = "IMSVS.PROD.BMP(AVR8020K)"
...
output_filename = "%s - AVR8020K.xls" % (datetime.datetime.now.strftime("%Y.%m.%d"))
...
if runxls.lower() == 'y':
    ...
    # sys.argv = [sys.argv[0]]
    sys.argv = [sys.argv[0],'--output='+output_filename]
    execfile(__file__)