...
# outlist name
# FILE        = os.path.join(OUTLIST_DIR,'VRP1100')
_param = args.param.replace(' ','_')
is_runxls = args.runxls and args.runxls.lower() == 'y'
# outlist name
if is_runxls:
    FILE        = os.path.join(OUTLIST_DIR,'VRP1100-'+_param)
else:
    FILE        = os.path.join(OUTLIST_DIR,'VRP1100')

# jcl mainframe name
JCL         = "IMSVS.PROD.BMP(VRP1100)"
...
if runxls.lower() == 'y':
    ...
    sys.argv = ['','--input=VRP1100-'+_param,'--output=VRP1100-'+_param+'.xls']
    execfile(__file__)