...
parser.add_argument('--param', help='210101 120131')
...
# outlist name
if is_runxls:
    FILE        = os.path.join(OUTLIST_DIR,'GAJI-KEKJK-'+_param)
else:
    FILE        = os.path.join(OUTLIST_DIR,'GAJI-KEKJK')

# jcl mainframe name
JCL         = "IMSVS.PROD.BMP(GAJI)"
...
jcl_param   = { 'xy' : [8,8], 'val' : param, 'scroll':1 }
...
if is_runxls:
    ...
    os.chdir('export')
    # sys.argv = [sys.argv[0],'--input='+FILE,'--output='+FILE]
    sys.argv = ['','--input=GAJI-KEKJK-'+_param,'--output=GAJI-KEKJK-'+_param+'.xls']
    # sys.argv = [sys.argv[0]]
    execfile(__file__)